    ...
    // call account-cli to deposit and get the script hash when the deposition finished.
	  ...
	  let account_id_pattern = Regex::new(r"Your account id: (\d+)").unwrap();
		...
		let miner_deposit_stdout = account_cli()
			...
			.spawn().unwrap()
			.stdout.unwrap();
		let mut last_ckb_balance_line = BufReader::new(miner_deposit_stdout).lines()
		...
		let user1_deposit_stdout = account_cli()
			...
			.spawn().unwrap()
			.stdout.unwrap();
		last_ckb_balance_line = BufReader::new(user1_deposit_stdout).lines()
	    .filter_map(|line| line.ok())
			.filter_map(|line| {
				println!("{}", &line);
				// update account_script_hash
		...
		let mut _withdrawal_status = account_cli().arg("withdraw")
			.args(&["--rpc", &ckb_rpc])
		  .args(&["-p", &miner.private_key])
			.args(&["--owner-ckb-address", "ckt1qyqy84gfm9ljvqr69p0njfqullx5zy2hr9kq0pd3n5"]) // ckt1qyqy84gfm9ljvqr69p0njfqullx5zy2hr9kq0pd3n5
