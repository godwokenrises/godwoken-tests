...
export default function setupBatchPrepare(program: Command) {
  program
    .command('batch-prepare-sudt')
    .description("Prepare SUDT for derived accounts (generated from the main account's private-key)")
    .requiredOption('-p, --private-key <HEX_STRING>', 'main account private key')
...
export async function prepareForBatch(params: {
  ...
  const fromPrivateKey = addHexPrefix(params.fromPrivateKey);
...
async function transferSudtToDerivedAccounts(params: {
  ...
  const signer = new Secp256k1Signer(addHexPrefix(fromPrivateKey), provider, secp256k1Script);
  const fromCkbAddress = privateKeyToBlake160Address(addHexPrefix(fromPrivateKey), config.lumosConfig);
